...
func (con NavController) Index(c *gin.Context) {
	...
	*/
	...
	//navList := []models.Nav{}
	//var num1 int64
	//models.DB.Find(&navList).Count(&num1)
	//c.JSON(200, gin.H{
	//	"result": num1,
	//})
	...
	//models.DB.Exec("update user set username='update' where id=1")
	//models.DB.Exec("UPDATE user SET username = ? WHERE id = ?", "updatename", 1)

	//使用原生 sql查询 uid=2 的数据
	//userList := []models.User{}
	//models.DB.Raw("select id=2 from user").Scan(&userList)
	//c.JSON(200,gin.H{
	//	"result": userList,
	//})
